...
func (lc *TestsController) CreateOption(ctx *gin.Context) {
	testsRepository := lc.testsRepository

	var option repositories.CreatedOptionDTO

	if err := ctx.ShouldBind(&option); err != nil {
		message := utils.ValidationErrorResponse(err)
		ctx.JSON(http.StatusBadRequest, gin.H{
			"message": message,
		})
	...
	questionID := ctx.GetInt("question_id")

	if err := testsRepository.CreateOption(uint(questionID), option); err != nil {
		ctx.JSON(err.StatusCode, gin.H{
			"message": err.Message,
		})
		return
...
func (cc *TestsController) DeleteOption(ctx *gin.Context) {
	ID := ctx.Param("option_id")

	testsRepository := cc.testsRepository

	if err := testsRepository.DeleteOption(ID); err != nil {
		ctx.JSON(err.StatusCode, gin.H{
			"message": err.Message,
		})
		return
